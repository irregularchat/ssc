...
import {
  getDB,
  getBasesWithFilters,
  getDistinctBaseBranches,
  getDistinctBaseStates,
  getDistinctBaseRegions,
  deleteBase,
} from '~/lib/db.server'
import type { BaseFilters } from '~/lib/db.server'

export const meta: MetaFunction = () => {
  return [{ title: 'Bases - Admin - CPL' }]
}
...
const REGION_OPTIONS = ['CONUS', 'OCONUS', 'Pacific', 'Europe', 'Middle East']
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  const sort = url.searchParams.get('sort') as BaseFilters['sort']
  const direction = url.searchParams.get('dir') as BaseFilters['direction']
  ...
  const [basesData, branches, states, regions] = await Promise.all([
    getBasesWithFilters(db, filters),
    getDistinctBaseBranches(db),
    getDistinctBaseStates(db),
    getDistinctBaseRegions(db),
  ])
  ...
  return {
    ...
    states,
    regions: regions.length > 0 ? regions : REGION_OPTIONS,
    filters: {
...
export default function AdminBasesListPage() {
  const {
    bases,
    total,
    page,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-text-primary">Military Bases</h2>
          <p className="text-text-secondary text-sm mt-1">
            {total} base{total !== 1 ? 's' : ''} total
          </p>
        </div>
              ...
              {bases.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center text-text-muted">
                    No bases found. Try adjusting your filters.
                  </td>
                </tr>
              ) : (
                bases.map((base) => (
                  <tr
                    ...
                    className={`hover:bg-bg-subtle/50 transition-colors ${
                      deleteConfirmId === base.id ? 'bg-error/5' : ''
                    }`}
                  >
                    <td className="px-4 py-3">
                      <div>
                        <p className="font-medium text-text-primary">{base.name}</p>
                        {base.abbreviation && (
                        ...
                        <div className="flex flex-wrap gap-1 mt-1 md:hidden">
                          ...
                          {base.state && (
                            <span className="text-xs text-text-muted">{base.state}</span>
                          )}
                        </div>
                      </div>
                    </td>
                      ...
                      {deleteConfirmId === base.id ? (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="icon"
                            onClick={() => setDeleteConfirmId(base.id)}
                          >
